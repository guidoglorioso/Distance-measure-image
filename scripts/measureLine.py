...
def detectar_referencia_px(img : np.array, color_ref : str = "verde",ref :float = 5) -> tuple:
    ...
    # Detecto lineas
    _lineas = cv2.HoughLinesP(_img2, rho=1, theta=np.pi/360, threshold=200, minLineLength=180, maxLineGap=30)
...
def get_matriz(ruta_img:str,dim_cuadrado:float,dim_rejilla:float)->np.array:
    ...
    _resultado = resize_y_threshold(_img,(200,200),threshold=200)



    return grey2rgb(_resultado)
